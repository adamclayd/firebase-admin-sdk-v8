...
function runTests(emulated: boolean = false) {
  ((emulated && process.env.FIRESTORE_EMULATOR_RUNNING  === 'true') || !emulated ? describe : describe.skip)('Firestore E2E Tests ' + (emulated ? process.env.FIRESTORE_EMULATOR_RUNNING !== 'true' ? ' - Emulator is not running' : ' (Running on emulator)' : ' (Running live)'), () => {
    ...
    beforeAll(() => {
      ...
      const emulator = `127.0.0.1:${JSON.parse(fs.readFileSync('firebase.json', 'utf-8')).emulators?.firestore?.port}`
      ...
      initializeApp(!emulated ? {
        ...
        firestoreEmulatorHost: emulator,
      });
    });
