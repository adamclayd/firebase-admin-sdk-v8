...
/**
 * Test resumable uploads for emulator e2e
 */
const emuRunning = process?.env?.STORAGE_EMULATOR_RUNNING === 'true';
(emuRunning ? describe : describe.skip)(`Resumable Upload E2E Emulator Tests ${!emuRunning ? ' - emulator is not running' : ''}`, () => {
  const TEST_PREFIX = 'e2e-resumable-';
  const testFiles: string[] = [];

  beforeAll(() => {
    const port = JSON.parse(String.fromCharCode(...new Uint8Array(fs.readFileSync(path.join(__dirname, '../../firebase.json'))))).emulators.storage.port;
