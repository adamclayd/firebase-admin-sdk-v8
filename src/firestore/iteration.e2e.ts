...
import { getEmulatorHost, isEmulatorRunning } from '../e2e-emulators';
...
function runTests(emulated: boolean = false) {
  ((emulated && isEmulatorRunning('firestore')) || !emulated ? describe : describe.skip)('Firestore Iteration E2E ' + (emulated ? !isEmulatorRunning('firestore') ? ' - Skipping Emulator is not running' : ' (Running on emulator)' : ' (Running live)' ), () => {
    beforeAll(async () => {
      ...
      const emulator = getEmulatorHost('firestore');
