...
const emuRunning = process?.env?.FIRESTORE_EMULATOR_RUNNING === 'true'; 
...
(emuRunning ? describe : describe.skip)(`getAll E2E Emulator Tests${emuRunning ? '' : ' - emulator not running'}`, () => {
  const TEST_COLLECTION = 'e2e-getall-tests';
  const timestamp = Date.now();
  const docIds = [ `doc1-${timestamp}`, `doc2-${timestamp}`, `doc3-${timestamp}` ];
  const port = JSON.parse(fs.readFileSync(path.join(__dirname, '../../firebase.json')).toString()).emulators.firestore.port;

  beforeAll(async () => {
    initializeApp({
      firestoreEmulatorHost: `127.0.0.1:${port}`,
...
(emuRunning ? describe : describe.skip)(`getAllByPaths E2E Emulator Tests${emuRunning ? '' : ' - emulator not running'}`, () => {
  const timestamp = Date.now();
  const COLLECTION_A = 'e2e-bypath-a';
  const COLLECTION_B = 'e2e-bypath-b';
  const docIdA = `docA-${timestamp}`;
